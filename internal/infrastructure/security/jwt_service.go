...
func NewJWTService(config valueobject.JWTConfig) service.JWTService {
...
func (j *JWTServiceImpl) GenerateTokens(userID, email string, roles []string) (*valueobject.TokenPair, error) {
	now := time.Now()

	// 生成访问令牌
	accessToken, err := j.generateToken(userID, email, roles, valueobject.TokenTypeAccess, now.Add(j.config.AccessTokenExpiry))
	if err != nil {
	...
	// 生成刷新令牌
	refreshToken, err := j.generateToken(userID, email, roles, valueobject.TokenTypeRefresh, now.Add(j.config.RefreshTokenExpiry))
	if err != nil {
...
func (j *JWTServiceImpl) RefreshToken(refreshToken string) (*valueobject.TokenPair, error) {
	// 解析刷新令牌
	token, err := jwt.ParseWithClaims(refreshToken, &valueobject.Claims{}, func(token *jwt.Token) (interface{}, error) {
		if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, fmt.Errorf("unexpected signing method: %v", token.Header["alg"])
		}
		return []byte(j.config.Secret), nil
...
func (j *JWTServiceImpl) RevokeToken(tokenString string) error {
	// 简单实现：记录日志
